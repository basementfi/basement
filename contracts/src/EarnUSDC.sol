...
/// @title EarnUSDC
/// @notice ERC-4626 vault that wraps a Morpho ERC-4626 vault.
///         Uses Morpho-style share-dilution fee: on every interaction,
///         yield is computed and new shares are minted to the treasury
///         (10% of yield). This keeps withdrawals clean — no USDC deducted.
contract EarnUSDC is ERC4626, Ownable, ReentrancyGuard, Pausable {
    ...
    /// @notice Tracks each user's deposited principal (in USDC, for UX)
    ...
    ///      attack 1 000 000× more expensive. Matches USDC's 6-decimal precision.
    ...
    /// @notice Total USDC value held in Morpho on behalf of this vault
    ...
    /// @dev Forward USDC into Morpho. Fees are already accrued by the public
    ...
    /// @dev Redeem from Morpho, send assets to receiver — no USDC fee deducted.
    ...
    ///      that many USDC from Morpho; when called via redeem() assets is the preview
    ...
    function _withdraw(
        ...
        // Withdraw exactly `assets` USDC from Morpho (covers both withdraw & redeem paths)
