...
/// @title EarnBTC
/// @notice ERC-4626 vault that wraps a Morpho ERC-4626 cbBTC vault (e.g. Gauntlet cbBTC Core).
///         Uses Morpho-style share-dilution fee: on every interaction, yield is computed and new
///         shares are minted to the treasury (10% of yield). Withdrawals stay clean — no cbBTC
///         deducted. Identical mechanism to EarnUSDC / EarnETH, with cbBTC as the asset.
contract EarnBTC is ERC4626, Ownable, ReentrancyGuard, Pausable {
    ...
    /// @notice The underlying Morpho ERC-4626 cbBTC vault
    ...
    /// @notice Tracks each user's deposited principal (in cbBTC, for UX)
    ...
    constructor(
        ...
        ERC4626(_cbbtc)
        ERC20("EarnBTC", "EarnBTC")
        Ownable(msg.sender)
    {
        require(_treasury != address(0), "Invalid treasury");
        require(_performanceFee <= MAX_FEE, "Fee too high");
        require(_morphoVault.asset() == address(_cbbtc), "asset mismatch");
    ...
    /// @dev Fees are accrued at the START of every public entry point, BEFORE OpenZeppelin prices
    ///      the trade via preview*(). This mirrors MetaMorpho and ensures the fee shares minted to the
    ///      treasury are already part of totalSupply when the depositor's/redeemer's share amount is
    ///      computed. Deposits/mints are blocked while paused; withdrawals/redeems stay open.
    ...
    /// @dev Adds 10^6 virtual shares, making the first-depositor inflation attack 1 000 000× more
    ///      expensive. Same offset as EarnUSDC / EarnETH.
    ...
    /// @notice Total cbBTC value held in Morpho on behalf of this vault
    ...
    /// @dev Forward cbBTC into Morpho. Fees are already accrued by the public entry point.
    function _deposit(
        address caller,
        address receiver,
        uint256 assets,
    ...
    /// @dev Redeem from Morpho, send assets to receiver — no cbBTC fee deducted. `assets` is honoured
    ///      for both the withdraw() and redeem() paths (shares == previewWithdraw(assets)).
    function _withdraw(
        address caller,
        address receiver,
        address owner,
    ...
    /// @dev Moves a proportional slice of the sender's tracked principal to the recipient on transfer.
    ///      Mints/burns are handled by _deposit/_withdraw, so skip them here to avoid double-counting.
